...
def main_effects(f):
    ...
    a = sns.boxplot(ax=axes[5], x=df.columns[group_column_metaphor], y=df.columns[response_column],
                    data=df, color='k', showfliers=False, order=order_metaphor, fill=False)
    sns.stripplot(ax=a, x=df.columns[group_column_metaphor], y=df.columns[response_column],
                  data=df, palette=['lightgreen', 'forestgreen'], order=order_metaphor)
    axes[5].yaxis.set_visible(False)
    axes[5].set_xlabel("Metaphor")  # Set custom label for the outer x-axis label
    axes[5].set_xticklabels(["Push", "Pull"])

    a = sns.boxplot(ax=axes[7], x=df.columns[group_column_gender], y=df.columns[response_column], fill=False,
...
subfigs = fig.subfigures(1, 2, width_ratios=(7, 3))
...
axess.set_ylabel('Distance (cm)', fontsize=18)
...
add_significance_bar(0, 2, 2650, 2600, "", axess)
add_significance_bar(1, 2, 2450, 2400, "", axess)
add_significance_bar(0, 1, 2550, 2500, "", axess)
add_significance_bar(2, 3, 2550, 2500, "", axess)
...
axess.text(2.5, -270, 'Vertical', ha='center', va='center', fontsize=18)
axess.text(0.5, -270, 'Horizontal', ha='center', va='center', fontsize=18)

# Adjust y-axis limits to make space for the text
axess.set_ylim(top=2700, bottom=0)
...
# plt.show()
plt.savefig("prelim_figures/dist_all.png")
