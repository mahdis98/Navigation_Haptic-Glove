...
def main_effects(axes):
    ...
    ax = sns.boxplot(ax=axes, data=data_h, x="Metaphor Approach", y="in zone percentage", showfliers=False, color='k',
                     fill=False, hue="Gender",
                     width=0.8, order=['push_two_tactor', 'push_worst_axis', 'pull_two_tactor', 'pull_worst_axis'],
                     gap=0.2)
    sns.stripplot(ax=ax, data=data_h, x="Metaphor Approach", y="in zone percentage",
    ...
    axes.text(0.5, -15, 'Push', ha='center', va='center', fontsize=18, weight='bold')
    axes.text(2.5, -15, 'Pull', ha='center', va='center', fontsize=18, weight='bold')
    # axes.text(1.5, -30, 'Vertical', ha='center', va='center', fontsize=18, weight='bold')

    # Adjust y-axis limits to make space for the text
    axes.set_ylim(top=105, bottom=-3)
    axes.set_ylabel('Percentage of Time in Critical Area (%)', fontsize=18)
...
# Create box plots using Seaborn
ax = sns.boxplot(ax=axess, data=data_v, x="Metaphor Approach", y="in zone percentage", showfliers=False, legend=False,
                 color='k', fill=False, hue="Gender",
                 width=0.8, order=['push_two_tactor', 'push_worst_axis', 'pull_two_tactor', 'pull_worst_axis'], gap=0.2)
sns.stripplot(ax=ax, data=data_v, x="Metaphor Approach", y="in zone percentage",
              order=['push_two_tactor', 'push_worst_axis', 'pull_two_tactor', 'pull_worst_axis'],
              palette=['blue', 'red'], hue="Gender", dodge=True)
...
axess.text(0.5, -15, 'Push', ha='center', va='center', fontsize=18, weight='bold')
axess.text(2.5, -15, 'Pull', ha='center', va='center', fontsize=18, weight='bold')
axess.set_title("Vertical", fontsize=25)
# Adjust y-axis limits to make space for the text
axess.set_ylim(top=105, bottom=-3)
...
plt.savefig("Prelim_figures_final/percentage_all.png", bbox_inches='tight')  # Save the plot with legend outside the plot
plt.show()
