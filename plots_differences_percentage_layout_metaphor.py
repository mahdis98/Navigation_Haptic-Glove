...
anova_result = f_oneway(y_data[group_data == 'vertical_push'],
...
ax = sns.boxplot(x=group_data, y=y_data, color = 'white', showfliers=False,
                 width=0.3,
                 order=['vertical_push', 'vertical_pull', 'horizontal_push', 'horizontal_pull'], legend=False)

# Set labels and title
plt.ylabel('Proportion of Time Spent in the Area', fontsize=18)
plt.xlabel('')
...
add_significance_bar(2, 3, 1.1, 1.05, tukey_result.pvalues[0])
add_significance_bar(1, 2, 1.2, 1.15, tukey_result.pvalues[0])
add_significance_bar(0, 2, 1.32, 1.27, tukey_result.pvalues[0])
add_significance_bar(1, 3, 1.5, 1.45, tukey_result.pvalues[0])
add_significance_bar(0, 3, 1.63, 1.58, 0.0002)
...
plt.ylim(top=1.8, bottom=-0.05)

# Get current figure size
fig_height = plt.gcf().get_figheight()

# Manually set the y-axis position for the text labels
plt.text(0.5, -0.05 * fig_height, 'Vertical', ha='center', va='center', fontsize=18)
plt.text(2.5, -0.05 * fig_height, 'Horizontal', ha='center', va='center', fontsize=18)
