...
# Perform ANOVA
anova_result = f_oneway(y_data[group_data == 'Female'].dropna(),
                        y_data[group_data == 'Male'].dropna())
...
ax = sns.boxplot(x=group_data, y=y_data, palette='Set2', showfliers=False,
                 width=0.6, dodge=True, gap=.1)  # Adjust the width and dodge parameters
...
def add_significance_bar(x1, x2, bar_height, bar_tips, p_value):
    ...
    plt.text((x1 + x2) * 0.5, text_height, sig_symbol, ha='center', va='bottom', c='k', fontsize=18)
...
add_significance_bar(0, 1, 120, 117, 0)

# Adjust y-axis limits to make space for the text
plt.ylim(top=130, bottom=-0.2)
