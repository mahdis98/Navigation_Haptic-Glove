...
fig, axes = plt.subplots(1, 9, figsize=(12, 8), sharey=True,
...
sns.boxplot(ax=axes[3], x=df.columns[group_column_8], y=df.columns[response_column],
            data=df, palette=['lightcoral', 'firebrick'], showfliers=False, order=order_8)
...
sns.boxplot(ax=axes[5], x=df.columns[group_column_metaphor], y=df.columns[response_column],
            data=df, palette=['lightgreen', 'forestgreen'], showfliers=False, order=order_metaphor)
axes[5].yaxis.set_visible(False)
axes[5].set_xlabel("Metaphor")  # Set custom label for the outer x-axis label
axes[5].set_xticklabels(["Pull", "Push"])

sns.boxplot(ax=axes[7], x=df.columns[group_column_gender], y=df.columns[response_column],
            data=df, palette=['lightsalmon', 'orangered'], showfliers=False, order=order_gender)
...
# Show the plot
plt.subplots_adjust(left=0.08, right=0.975)
# plt.show()
plt.savefig("prelim_figures/time_main_effects.png")
